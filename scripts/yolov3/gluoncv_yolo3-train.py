...
def parse_args():
    ...
    parser.add_argument('--obj-metric', type=str, default='mAP',
                        help="Select an objective metric to optimize using a SageMaker tuner: 'mAP','Objective-Loss','Box-Center-Loss','Box-Scale-Loss','Class-Loss'")
    parser.add_argument('--data-shape', type=int, default=416,
                        help="Input data shape for evaluation, use 320, 416, 608... " +
                             "Training is with random shapes from (320 to 608).")
    parser.add_argument('--opt', type=str, default='adam',
                        help="Optimzer to use for training. Currently only sgd or adam is supported.")
    parser.add_argument('--batch-size', type=int, default=32,
                        help='Training mini-batch size')
    parser.add_argument('--pre-trained-weights', type=str, default='VOC',
                        help='Set COCO or VOC pre-trained weights. Otherwise, set NONE to disable pre-training.')
    parser.add_argument('--data-prefix', type=str, default='',
                        help='Image folder prefix relative to train or test channel as referenced by LST files.')                   
    ...
    if args.local != True :
        ...
        if args.test == None : args.test = os.environ['SM_CHANNEL_TEST']
    ...
    global CLASSES_FILE
    CLASSES_FILE = os.path.join(args.train, 'classes.csv')
        
    return args

# This script is only implemented to support RecordIO format. In addition to RecordIO data in
# the train and test channel, the script expects the following files at the root:
#   1. List files for training and validation sets named train.lst and val.lst at the root of the train and 
#      test channels respectively: 
#      (https://mxnet.incubator.apache.org/versions/master/faq/recordio.html#step-1-make-an-image-list-file)
#   2. classes.csv which should be a comma-delimited list of two columns consisting of the class ids and 
#      names. This file should be located at the root of the train channel.
def load_data():

    train_dataset = gcv.data.RecordFileDetection(os.path.join(args.train,'train.rec'))
    val_dataset = gcv.data.RecordFileDetection(os.path.join(args.test,'val.rec'))
...
def train(net, train_data, val_data, eval_metric, ctx, args):
    ...
    if args.opt == 'sgd' :
        ...
        raise NotImplementedError('Optimizer option: {} not implemented.'.format(args.opt))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not (epoch + 1) % args.val_interval:
            ...
            EVAL_METRICS['mAP'] = mean_ap[len(mean_ap)-1]

            val_msg = '\n'.join(['{}={}'.format(k, v) for k, v in zip(map_name, mean_ap)])
            logger.info('[Epoch {}] Validation: \n{}'.format(epoch, val_msg))
            logger.info('Target Objective: {} = {:.8f}'.format(args.obj_metric, EVAL_METRICS[args.obj_metric]))
...
if __name__ == '__main__':
    ...
    try:
        ...
        ctx = [mx.cpu()]
        #print('Using CPU. GPU device was not detected')
   ...
   #     json.dump(MODEL_INFO,model_meta)
    ...
    if args.pre_trained_weights == 'NONE' :
        TRANSFER = None
